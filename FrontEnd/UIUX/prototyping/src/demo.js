...
function custSelectChange() {
  ...
  $("select").on("changed.bs.select", function (e, clickedIndex, newValue, oldValue) {
    if (this.id) {
      var selectedId = this.id, selectedValue = this.value;
...
function custBootstrapSelect() {
  $('.selectpicker--select-all')
    .each(function (i, e) {
      const opts = $(this).clone().html().trim();
      const html = `
      ...
      $(this)
        ...
        .attr('multiple', true)
        .attr('data-select-all', true)
        .val('').selectpicker('render');
    });
  $('.selectpicker--select-all')
    .on('changed.bs.select', function (event, clickedIndex, newValue, oldValue) {
      ...
      if ($(this).find(':selected[value="全選"]').length > 0) {
        ...
        $(this)
          .selectpicker('val', [...allValueList])
          .selectpicker('toggle').selectpicker('toggle');
...
function stickInfo(preTarget) {
  $('.nav-pills').on('shown.bs.tab', 'a', function (e) {
    var shouldScrollTop = $(preTarget).attr('href') == $(e.target).attr('href') ? false :
      true
    if (preTarget) {
  ...
  switch (id) {
    ...
    case 'admin_sidebar':
      ...
      if ($('#admin_sidebar .nav-link.active').length > 1) {
        $('#admin_sidebar .nav-link.active').each((i, e) => { 
...
function initNav() {
  ( window.getAcct() === "manager" ||
    (window.getAcct() === "admin" && parse_url_filename().indexOf('M_') === 0)
  ) ? $('#salesJob').removeClass('displayno') : null;
}
...
function customerDebitLoadMore({count, replaceText}) {
  count = count || 10;
  let cloneHTML = $(this).closest('.card').find('tbody tr:first-child').html();
  let returnHTML = '';
  ...
  if ($(this).closest('.card').find('.card-loading').length === 0) {
    $(this).closest('.card')
      ...
      .append('<div class="card-loading"><div class="card-loading-icon">' + loadingHTML() + '</div></div>');
  }
  ...
  setTimeout(() => {
    $(this).closest('.card').find('.card-loading').removeClass('active');
    $(this).closest('.card').find('tbody').append(returnHTML);
  }, 3000);
}
...
GlobalSearchKey.prototype.listKeyCtrl = function () {
  let _this = this;
  $(document).on('keyup', _this.target, function (e) {
    const activeItem = $(_this.target).siblings('.global-search-select').find('a:focus');
    const firstItem = $(_this.target).siblings('.global-search-select').find('a:first-child');
    const lastItem = $(_this.target).siblings('.global-search-select').find('a:last-child');
...
function mdCreateSysSetOpen() {
  $('#modalCreateSysSetting').on('shown.bs.modal', function() {
    ...
    const nowVal = $(this).find('[name="sysItemName"]:first').val();
...
function modalCreateSysSettingSubmit(e) {
  e.preventDefault();
  // console.log(e);
  const code = $(e.target).closest('.modal').find('[name="sysItemCode"]:first').val();
  const label = $(e.target).closest('.modal').find('[name="sysItemName"]:first').val();
  const id = $(e.target).closest('.modal').find('[name="sysItemFrom"]:first').val();
  // 
  const labelOri = $(e.target).closest('.modal').find('[name="sysItemNameOri"]:first').val();
  ...
  if(code && label) {
    ...
    if($('#modalCreateSysSetting').hasClass('modal-edit') && id && labelOri) {
      ...
      $('[data-toggle="list"][href="'+id+'"]')
        .closest('li').find('.edit-title:first')
        .addClass('under-review-edit')
        .attr('onclick', "modalCreateSysSettingEditUnderReview('"+id+"', '"+code+"', '"+label+"', '"+labelOri+"')");
    }
    // create new one
    else {
      const newId = $('#sysSidebar>li').length;
      $('#sysSidebar').append(`
        <li>
...
function modalCreateSysSettingEditUnderReview(id, code, label, labelOri, type) {
  ...
  if (type === 'review') {
    $('#modalCreateSysSetting')
      .addClass('modal-m-review');
  }
  $('#modalCreateSysSetting')
    .removeClass('modal-edit')
    .addClass('modal-under-review')
    .modal('show');
}
...
modalCreateSysItemSetting.create = function(ele) {
  // console.log(ele,123)
  const name = $(ele).closest('.modal').find('[name="sysItemName"]').val();
  const code = $(ele).closest('.modal').find('[name="sysItemCode"]').val();
  const val = $(ele).closest('.modal').find('[name="sysItemValue"]').val();
  const nextly = $(ele).closest('.modal').find('[name="hasNextLayer"]').prop('checked');  
  const rand = Math.random()*200;
  const li = `
        <li>
          <a role="tab" data-toggle="list" class="list-title" href="#sysSidebar_1000">${name}</a>
        </li>`;
  const activeTarget = '#' + $('#pills-tabContent .tab-pane.show.active [id^="sysSidebar"].active').attr('id');
  ...
  if(nextly) {
    ...
    const newCollapseListId = 'listCollapse_sysSidebar_' + Math.random() * 300;
    if($('[href="'+activeTarget+'"]').closest('li').find('ul').length === 0) {
      $('[href="'+activeTarget+'"]').closest('li').find('a:first').addClass('list-collapse')
      .attr('data-list-collapse-target', '#' + newCollapseListId);

      $('[href="'+activeTarget+'"]').closest('li')
        .append('<ul clas="collapse show" id="'+newCollapseListId+'"></ul>')
    }
...
modalCreateSysItemSetting.editSubmit = function () {
  // 此狀態下如果有送出則一定會是待審核
  $('#modalCreateSysItemSettingEdit [type="submit"]')
    .on('click', function() {
      if($('#modalCreateSysItemSettingEdit').hasClass('modal-under-review') || $('#modalCreateSysItemSettingEdit').hasClass('modal-123')) {
        return;
      }
      const dataFrom = $('#modalCreateSysItemSettingEdit [name="sysItemFrom"]').val();
      const sidebarFrom = $('#' + dataFrom).closest('.tab-pane.active').attr('id');
...
modalCreateSysDrop.create = function (type) {
  type = type ? type : 'sub';
  const dataFrom = $(this).closest('.tab-pane.active').attr('id');
...
modalCreateSysDrop.createSubmit = function () {
  // new是sidebar的新增
  // sub是右方項目的新增
  const createStatus = $(this).closest('.modal').hasClass('modal-status-new') ? 'new' : 'sub';
  const name = $(this).closest('.modal').find('[name="sysItemName"]').val();
  const code = $(this).closest('.modal').find('[name="sysItemCode"]').val();
  const val = $(this).closest('.modal').find('[name="sysItemValue"]').val();
  const nextly = $(this).closest('.modal').find('[name="hasNextLayer"]').prop('checked');
  const rand = Math.random().toString();
  const randId = rand.substr(rand.length - 8);  
  const contentAppendTarget = $('#pills-notice .tab-content:first');
  ...
  if (createStatus === 'new') {
    ...
    const sideBarAppendTarget = $('#sysSidebar1')
    $(sideBarAppendTarget).append(li);
  } else {
    const contentTargetFrom = $('#pills-notice .tab-content .tab-pane.active:first').attr('id');
    const parentLi = $('[href="#' + contentTargetFrom + '"][data-toggle="list"]').closest('li');
    console.log(contentTargetFrom, parentLi);
    
    if (nextly) {
      if ($(parentLi).find('ul').length === 0) {
        // console.log(1)
        $('[href="#' + contentTargetFrom + '"][data-toggle="list"]')
          .attr('data-list-collapse-target', '#listCollapse_sysSidebar_' + randId)
          .addClass('list-collapse');
          // .addClass('collapsed');
        $(parentLi).append(`
...
modalCreateSysDrop.edit = function (type) {
  const rand = Math.random().toString();
  const randId = rand.substr(rand.length - 8);  
  const modalStatusClassName = (type === 'underReveiew' || type === 'reveiew') ? 'modal-under-review' : ( (type === '123') ? 'modal-123' : 'modal-edit' );
  const dataFrom = (type === 'new') ? { type: 'new', from: $(this).closest('li').find('.list-collapse').attr('href') } : { type: 'sub', from: $(this).closest('div').attr('id')};
...
modalCreateSysDrop.editSubmit = function () {
  const editStatus = $(this).closest('.modal').hasClass('modal-under-review') ? 'underReveiew' : ($(this).closest('.modal').hasClass('modal-123') ? '123' : 'edit');

  if (editStatus === 'underReveiew' || editStatus === '123') {
    return;
  }

  const dataFrom = JSON.parse($(this).closest('.modal').find('[name="sysItemFrom"]').val());
  ...
  if (dataFrom.type === 'new') { 
    ...
    // 1. "檢視"按鈕
    // 2. sidebar title紅點
    const sidebarTitleFrom = $('#' + dataFrom.from).closest('.tab-pane.active').attr('id');
    $('#' + dataFrom.from).html(`
      <label class="col-form-label"><span class="tag square red">待覆核</span></label>
      <div class="btn-group">
...
modalCreateSysDrop.review = function (isOk) {
  if (isOk) {
    ...
    $('#modalSysBack')
      .find('[name="sysItemFrom"]:first')
      .val('modalCreateSysDropDSettingEdit');
...
modalCreateSysDrop.close = function () {
  $('#modalCreateSysDropDSetting').on('hidden.bs.modal', function () {
    $('#modalCreateSysDropDSetting')
      .removeClass('modal-status-new')
      .removeClass('modal-status-sub');
  ...
  $('#modalCreateSysDropDSettingEdit').on('hidden.bs.modal', function() {
    $('#modalCreateSysDropDSettingEdit')
      .removeClass('modal-m-review')  
      .removeClass('modal-under-review')
      .removeClass('modal-edit');
...
modalCreateSysUpload.open = function (type) {
  const modal = '#modalCreateSysUploadSetting';
  console.log(type);
  switch (type) {
    case 'new':
...
modalCreateSysUpload.submit = function () {
  const modal = '#modalCreateSysUploadSetting';
  const type = $(modal).hasClass('modal-edit') ? 'edit' : ($(modal).hasClass('modal-under-review') ? 'underReview' : 'new');
  switch (type) {
    case 'new':
      $('[href="#sysSidebar3_2"]').removeClass('displayno');  
      break;
    case 'edit':
      const target = $(modal + ' [name="sysItemFrom"]').val();
...
modalCreateSysUpload.close = function (params) {
  $('#modalCreateSysUploadSetting').on('hidden.bs.modal', function () {
    $('#modalCreateSysUploadSetting')
      .removeClass('modal-new')  
      .removeClass('modal-edit')
      .removeClass('modal-under-review');
...
var modalSysBack = {
  submit: function () { 
    ...
    if (target === 'modalCreateSysDropDSettingEdit') {
      ...
      $('#sysSidebar1_1 i[data-toggle="popover"]')
  ...
  watch: function() {
    $(document).on('click', '.list-collapse[data-list-collapse-target]', function(){
      ...
      if($(listCollapseTarget).hasClass('show')) {
        ...
        $(listCollapseTarget)
          .css('height', $(listCollapseTarget).height())
          .css('height', '')
          .addClass('collapsing')
        ...
        $(listCollapseTarget)
          .addClass('collapsing')
          .removeClass('collapse')
          .css('height', listCollapseTargetHeight);
    ...
    $(document).on('click', '.list-title', function(){
      ...
      setTimeout(()=>{
        ...
        $(_this).closest('li').removeClass('current')
          .find('.list-title:first').addClass('active').addClass('show');
      }, 0);
    });
  }
}
...
var popoverHoverStayOpt = {
  ...
  content: function () {
    const mainTarget = $(this).attr('data-target') || $(this).attr('href');
...
function popoverHoverStay() {
  ...
  $(document)
    .on("mouseenter", ".pop-hover-stay", function () {
        var _this = this;
        $(this).popover("show");
